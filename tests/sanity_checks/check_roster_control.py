...
def _run(mode, ids, coords, uavs, cap, rnd=3):
    s = ClientSelector(ids)
    return s.select(
        client_ids=ids,
        compute_times={c: 1.0 for c in ids},
        energies={c: 1.0 for c in ids},
        utilities={c: 0.5 for c in ids},
        reputations={c: 0.5 for c in ids},
...
def rosters_differ_when_slots_are_slack():
    """The regime the confound lives in: few clients, many UAVs."""
