...
echo "Full pipeline:   HF_TOKEN=hf_xxx scripts/run_gcp.sh"
echo "Smoke check:     HF_TOKEN=hf_xxx SMOKE=1 scripts/run_gcp.sh"
echo "(run_gcp.sh detaches itself from the terminal automatically — it"
echo " prints a PID and log path, then you can close the SSH session.)"
echo "(Killed partway through? Just relaunch the same command — every step"
echo " checkpoints per job and resumes instead of restarting from scratch.)"
echo "(HF runtime tuning — HF_MAX_WORKERS, HF_HUB_DOWNLOAD_TIMEOUT, dataset"
echo " revision pin — is exported by run_gcp.sh, not here: exports from this"
echo " setup script would not survive into your shell anyway.)"
